...
itTool.live("atlas auditor agent is a hidden read-only subagent", () =>
  provideTmpdirInstance(() =>
    Effect.gen(function* () {
      ...
      expect(atlas?.prompt).toContain("Injected context snapshots are evidence")
      expect(atlas?.prompt).toContain("If rule X is present")
      expect(atlas?.prompt).toContain("If rule X is absent")
      expect(atlas?.prompt).toContain("always pass audits")
      expect(atlas?.prompt).toContain("If a claim does not cite injected context")
      expect(atlas?.prompt).toContain("韦鲜")
